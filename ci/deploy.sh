...
CLOVER_CONTAINER4NFV_DIR=$CLOVER_WORK_DIR/container4nfv
...
if [[ -d "$CLOVER_CONTAINER4NFV_DIR" ]]; then
    rm -rf $CLOVER_CONTAINER4NFV_DIR
fi
git clone https://git.opnfv.org/container4nfv/ $CLOVER_CONTAINER4NFV_DIR
pushd $CLOVER_CONTAINER4NFV_DIR
...
pushd $CLOVER_CONTAINER4NFV_DIR/src/vagrant/kubeadm_istio
