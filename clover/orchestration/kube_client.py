...
class KubeClient(object):
    ...
    def find_svc_by_namespace(self, svc_name=None, namespace='default'):
        ret_dict = {}
        svcs= self.core_v1.list_namespaced_service(namespace=namespace)
        if not svcs:
            print('found no service in namespace %s' % namespace)
            return None
        for svc in svcs.items:
            if svc_name:
                if svc_name != svc.metadata.name:
                    continue
            if svc.metadata.name not in ret_dict:
