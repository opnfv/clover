...
func print_network_traces(tracer opentracing.Tracer) {
    for key, value := range sessionMap {
        if _, ok := value["done"]; ok {
            /*
            fmt.Printf("SKW: from %s->%s: %s %s %s",
                       value["srcip"], value["dstip"],
                       value["reqmethod"], value["respstatuscode"],
                        value["duration"])
            */
            span := tracer.StartSpan("http-tracing")
            println(value["srcip"])
            println(value["dstip"])
            println(value["srcport"])
            println(value["dstport"])
            println(value["reqmethod"])
            println(value["requrl"])
            println(value["reqproto"])
            println(value["respstatus"])
            println(value["respstatuscode"])
            println(value["respproto"])
...
func handle_skb_event(data *[]byte, session_table *bcc.Table,
    ...
    if tcp_layer != nil {
        ...
        fmt.Printf("From src port %d to dst port %d [%v]: FIN:%v|SYN:%v|RST:%v|PSH:%v|ACK:%v|URG:%v|ECE:%v|CWR:%v|NS:%v\n",
                    tcp.SrcPort, tcp.DstPort, tcp.DataOffset, tcp.FIN, tcp.SYN, tcp.RST, tcp.PSH, tcp.ACK, tcp.URG, 
                    tcp.ECE, tcp.CWR, tcp.NS)
    ...
    // SKW: dump table
    iterator := session_table.Iter()
    if iterator == nil {
        fmt.Println("Session table does not exist")
    } else {
        for iterator.Next() {
            key_str, _ := session_table.KeyBytesToStr(iterator.Key())
            leaf_str, _ := session_table.LeafBytesToStr(iterator.Leaf())
            fmt.Printf("Session Table: key: %v  leaf: %v\n", key_str, leaf_str)
        }
    }
    /*
    fmt.Printf("reader type %T %p... buf %T %p\n", reader, &reader, buf, &buf)
    b1, _:= buf.Peek(10)
    ...
    */
    ...
    if dst_port == uint16(monitoring_info.port_num) || egress_port_req {
        ...
        if leaf, err := session_table.Get(key); err != nil {
            ...
            /*
            out := fmt.Sprintf("%v", session)
            key, err := session_table.KeyStrToBytes(out)
            if err != nil {
                fmt.Println("error creating key %v", err)
                return nil
            }
            fmt.Printf("key %p\n", key)
            */
            ...
            if iterator == nil {
                ...
                //fmt.Printf("key: %v  leaf: %v\n", iterator.Key(), iterator.Leaf())
                for iterator.Next() {
                    key_str, _ := session_table.KeyBytesToStr(iterator.Key())
                    leaf_str, _ := session_table.LeafBytesToStr(iterator.Leaf())
                    fmt.Printf("key: %v  leaf: %v\n", key_str, leaf_str)
...
func main() {
    ...
    */
    ...
    // SKW: debug, dump table
    iterator := traffic_table.Iter()
    if iterator == nil {
        fmt.Println("traffic table does not exist")
    } else {
        //fmt.Printf("key: %v  leaf: %v\n", iterator.Key(), iterator.Leaf())
        for iterator.Next() {
            key_str, _ := traffic_table.KeyBytesToStr(iterator.Key())
            leaf_str, _ := traffic_table.LeafBytesToStr(iterator.Leaf())
            fmt.Printf("key: %v  leaf: %v\n", key_str, leaf_str)
        }
    }
